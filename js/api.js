// api-base.js
const BASE_URL = "https://ec2-3-147-64-107.us-east-2.compute.amazonaws.com:3000";
...
export const apiBase = {
  ...
  async cadastrar(endpoint, dados) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Erro ao cadastrar ${endpoint}: ${response.statusText}`
        );
      }
      return await response.json();
    } catch (error) {
  ...
  async atualizar(endpoint, id, dados) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Erro ao atualizar ${endpoint}: ${response.statusText}`
        );
      }
      return await response.json();
    } catch (error) {
